...
<script>
  ...
  export default {
    ...
    data: () => ({
       ...
       items: [],
       iniItems: [
         { id: 1, icon: 'mdi-pin', color: '#FFFF8D', value: '' },
         { id: 2, icon: 'mdi-balloon', color: '#FFF9C4', value: '' },
         { id: 3, icon: 'mdi-map-marker', color: '#B9F6CA', value: '' },
         { id: 4, icon: 'mdi-pin', color: '#FFCDD2', value: '' },
         { id: 5, icon: 'mdi-balloon', color: '#FFFF8D', value: '' },
         { id: 6, icon: 'mdi-pin', color: '#FFFF8D', value: '' },
       ],
       currentPageNo: '1',      
       currentPageContent: '',
       folderItems: [
  ...
  methods: {
      ...
      changePage(pageNo) {                
        ...
        for (let i=0; i<this.folderItems.length; i++){          
          this.folderItems[i].sel = (this.folderItems[i].id == pageNo)
        }   
      },
      ...
      // MODIFICATO: Salvataggio JSON pulito
      doSaveAsFile(){
        try {
          const dataJson = JSON.stringify(this.items, null, 2)        
          ...
          exportFileLink.download = this.currentPageNo + 'share.json'
      ...
      // MODIFICATO: Apertura JSON pulito senza password
      doOpenFile(){
        if (!confirm('Open file will Overwrite current Page, Confirm?') ) return
        const that = this
        const inputFile = document.createElement('input')
        inputFile.type = 'file'
        inputFile.accept = '.json, .123notes'
        inputFile.addEventListener('change', function(event){
           const jsonFile = event.target.files[0]
           const reader = new FileReader()    
           reader.onload = function(event){
              try {
                // Leggiamo direttamente il JSON senza decriptare
                const dencryptData = event.target.result
                that.items = JSON.parse(dencryptData)  
                that.saveNote(that.currentPageNo)
                that.setPageContent()
                alert("File caricato con successo!");
              } catch (e) {
                alert("Errore: Il file non è un JSON valido o è ancora criptato.");
              }
           }
